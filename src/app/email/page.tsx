...
export default async function DuesCollectionEmailPage() {
    ...
    <AppShell chapterName={user.chapter_name} userId={user.id}>
      <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 sm:py-10">
        <h1 className="mb-1 font-display text-3xl text-foreground">
          Dunn
          <span className="text-muted-foreground"> · Email · {period.name}</span>
        </h1>
        <p className="mb-6 text-sm text-muted-foreground">
          Compose a dues message from the roster emails already in Members.
        </p>
        <EmailComposer
